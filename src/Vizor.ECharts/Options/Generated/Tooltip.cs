...
public partial class Tooltip
{
    ...
    /// In this time, series index can be referred as {a0} , {a1} , or {a2} .
    ///  
    ...
    ///  
    /// The default value is false , which means that the tooltip DOM element will be one of a descendant of its echarts DOM container.
    ...
    /// <![CDATA[
    /// Since v5.5.0   
    /// Which DOM element to append the tooltip to.
    /// Only available when using renderMode  'html' .
    ///  
    ...
    ///  
    /// For such scenarios, appendTo can be specified.
    /// When it is a function, the interface is in the form of  (chartContainer: HTMLElement) => HTMLElement | undefined | null  
    /// which means it returns the node that the tooltip's DOM node should be added to.
    /// Returning undefined or null indicates that the default logic described above should be used.
    /// Returning an HTMLElement means that it should be added under that node.
    ///  
    /// Note that it also works when CSS transform used.
    /// ]]>
    /// </summary>
    [JsonPropertyName("appendTo")]
    //TODO: Type Warning: Failed to map property 'appendTo' in type 'tooltip' with types 'function,htmlelement,string'

...
public partial class ChartOptions
{
    ...
    ///    
    /// How to set filterMode is up to users, depending on the requirements and scenarios.
    /// Expirically:   
    ...
    /// dispatchAction can be used to render select-box programmatically.
    ...
    ///  
    /// brushLink is an array of seriesIndex es, which assigns the series that can be interacted.
    ...
    /// Elements with different types have different shape setting respectively.
    /// For example:  {
    ///     type: 'rect',
    ///     shape: {
    ///         x: 10,
    ...
    ///  
    /// The separate hover layer is used to avoid re-painting the whole canvas when hovering on elements.
    /// Instead, the hovered elements are rendered in a separate layer so that other elements don't have to be rendered again.
    ///  
    /// ECharts 2 use separate layer for all cases.
